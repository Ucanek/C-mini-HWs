...
/*
 * Minihomework 06: Malloc byte array 3
 * ---------------
 * Author: Lukas Rucka
 ...
 * 1. Implement function malloc_byte_array which returns allocated array of
 *   n + 256 uint8_t values.
 * 2. Zero-initialize the array.
 * 3. The array shall be preceded by (sizeof(uint8_t*)+sizeof(size_t)) bytes
 *    which store the pointer to the data including the service records
 ...
 * 4. This time, however, compute the service locations relative to pointer
 *    returned to caller, not relative to the pointer returned by malloc.
 *    That is, first compute the return value and afterward setup everything
 *    else.
 * 5. Complement malloc_byte_array with free_byte_array which takes pointer to
 *    the array obtained from malloc_byte_array and invokes free on the correct
 *    position.
 *
 * So to summarize, your solution should work as in task09, but this time work
 * with return pointer when assigning service data to array.
 *
