...
/*
 * Minihomework 09: Level sum of nodes in an n-ary tree
 * ----------------------------------------------------------------------------
 * Author: Adam Štěpánek
 * Email:  xstepan1@fi.muni.cz
 * N-ary tree by: Roman Lacko <xlacko1@fi.muni.cz>
 *
 ...
 * ----------------------------------------------------------------------------
 *
 * Write a recursive function 'levelsum' that returns the sum of levels
 * of nodes with value equal to the second parameter for a given n-ary tree.
 * 
 * The level of a node is the number of edges between itself and the root plus
 * one.
 * 
 * Example:
 *     [a]
 ...
 * d    | 3
 *
 * Given that [a] and [d] nodes have the desired value, resulting sum
 * will be 1 + 3 = 4.
 *
 * Note: Node's level = node's depth + 1.
 *
 * The 'node' structure used to represent a tree is the same as in task02.
 */
...
/**
 * Get the the sum of levels of nodes that contain a given value.
 * 
 * \param   the root node of an n-ary tree
 * \returns the level sum
 */
